- **A row in a series' log that stops partway through no longer takes the
  ...
  recorded and no warning printed — reachable today by pasting a two-line name
  into the alignment or ztrace rename box. No exception handler can reach that
  case; refusing the join would, but it would also make a row that can never be
  completed raise where it currently produces something, which is a change every
  reader of the log sees. It is pinned in
  `tests/test_editors_from_corrupt_history.py` as a known limitation, so the
