- **An error that keeps happening now opens one window instead of an endless
  ...
  and the line that raised it -- opens a window once per session. Every
  occurrence is still written to the log file, so nothing is lost: `Help > View
  log file` shows the repeats, and the window now says so.
