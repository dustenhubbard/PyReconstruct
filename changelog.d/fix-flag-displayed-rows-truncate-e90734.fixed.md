- **The flag and trace lists no longer hold flags and traces alive after a
  ...
  for the rows it created. A rebuild that shrank the list -- any of the
  filters, turning `Display resolved flags` back off, or a section change --
  left the entries past the last row in place, so the lists only ever grew and
