...
- **Fixed:** Importing from another copy of a series can no longer throw your tracing away without saying so. When you merge two copies of a series that were traced in parallel, the import used to be able to drop a trace with nothing to show for it — no flag, no log entry — and in some cases the trace it dropped was your own, quietly replaced by the incoming copy. Now a trace can only be discarded if it genuinely overlaps something kept from the other copy, or if the log records that it was deliberately deleted or renamed, and anything discarded always leaves behind both a flag and a log entry. Where the merge cannot tell which side is right, it now keeps both traces and flags the disagreement instead of picking a winner.
- **Changed:** "Check series histories" is now turned on by default when you import, so you will see conflicts that used to be resolved silently. Expect more conflict flags on your first import with this build, including new flags whose names start with "import-removed". This is not new damage and nothing extra is being lost: those disagreements were always in your data, and the import used to settle them behind your back. Traces that agree on both sides still merge quietly and produce no flags at all.
- **Fixed:** The "Overlap threshold" slider in the import window now actually does what it says. On the traces that matter — the ones that genuinely differ between the two copies — the slider was ignored and a fixed setting was always used instead. Turning it up to a stricter setting could even make the import silently drop your own version of a trace. It now applies to every comparison.
...
- **Improved:** Lasso selection and drawing are much faster on dense series, and large series hold far less memory. Selecting with the lasso on a dense automatic segmentation went from about fourteen seconds to about one in our own testing, and redrawing a densely traced frame is over one and a half times faster. Separately, trace measurements are now worked out when something asks for them instead of being carried around for every trace, which cuts the memory a large series holds for its traces by up to about three quarters.
