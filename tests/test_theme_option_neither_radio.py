...
theme matches none of the System/Light/Dark radios (all unselected).

``theme`` is a global QSettings-backed option shared across series, so a value
written by some other build (e.g. a named theme this dialog doesn't know)
can leave every radio unchecked. The setter bound the chosen mode only inside
if/elif, so ``accept()`` -> ``set()`` raised ``UnboundLocalError`` and took
down the whole dialog. The fix seeds the value with the current theme first.
...
def test_neither_radio_selected_preserves_current_theme(qapp, tmp_path):
    ...
    try:
        ...
        # System/Light/Dark radios, none selected
        w.responses = ([("system", False), ("light", False), ("dark", False)],)
...
def test_selecting_a_radio_still_sets_that_theme(qapp, tmp_path):
    ...
    dlg, w = _theme_widget(qapp, tmp_path)
    try:
        dlg.series = _ThemeStub("system")
        # System/Light/Dark radios, Dark selected
        w.responses = ([("system", False), ("light", False), ("dark", True)],)
        w.set()
        assert dlg.series.writes == ["dark"]
    finally:
        dlg.deleteLater()
